...
# GA-Net: Pizzuti
def ga_community_detection(graph, population, generation, r, crossover_prob, mutation_prob):
    ...
    while gen < generation:
        for i in range(int(np.floor(population / 10))):
            ...
            child = __uniform_crossover(dframe["chrom"][p1], dframe["chrom"][p2], crossover_prob)
            if len(child) == 0:
                continue
            child = __mutation(child, Adj, mutation_prob)
